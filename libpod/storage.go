...
type storageService struct {
	storageImageServer *Runtime
}

// getStorageService returns a storageService which can create container root
// filesystems from images
func getStorageService(runtime *Runtime) (*storageService, error) {
	return &storageService{storageImageServer: runtime}, nil
}
...
func (r *storageService) CreateContainerStorage(systemContext *types.SystemContext, imageName, imageID, containerName, containerID, mountLabel string) (ContainerInfo, error) {
	...
	ref, err := istorage.Transport.ParseStoreReference(r.storageImageServer.store, imageName)
	if err != nil {
	...
	//}
	fmt.Println("100")
	img, err := istorage.Transport.GetStoreImage(r.storageImageServer.store, ref)
	if err != nil {
		return ContainerInfo{}, err
	}
	...
	container, err := r.storageImageServer.store.CreateContainer(containerID, names, img.ID, "", string(mdata), nil)
	if err != nil {
		//if metadata.Pod {
	...
	defer func() {
		if err != nil {
			if err2 := r.storageImageServer.store.DeleteContainer(container.ID); err2 != nil {
				//if metadata.Pod {
	...
	names, err = r.storageImageServer.store.Names(container.LayerID)
	if err != nil {
		return ContainerInfo{}, err
	}
	names = append(names, layerName)
	err = r.storageImageServer.store.SetNames(container.LayerID, names)
	if err != nil {
		return ContainerInfo{}, err
	}

	// Find out where the container work directories are, so that we can return them.
	containerDir, err := r.storageImageServer.store.ContainerDirectory(container.ID)
	if err != nil {
		return ContainerInfo{}, err
	}
	//if metadata.Pod {
	...
	containerRunDir, err := r.storageImageServer.store.ContainerRunDirectory(container.ID)
	if err != nil {
		return ContainerInfo{}, err
	}
	//if metadata.Pod {
...
//func (r *storageService) CreatePodSandbox(systemContext *types.SystemContext, podName, podID, imageName, imageID, containerName, metadataName, uid, namespace string, attempt uint32, copyOptions *copy.Options) (ContainerInfo, error) {
...
//func (r *storageService) CreateContainer(systemContext *types.SystemContext, podName, podID, imageName, imageID, containerName, containerID, metadataName string, attempt uint32, mountLabel string, copyOptions *copy.Options) (ContainerInfo, error) {
...
//func (r *storageService) RemovePodSandbox(idOrName string) error {
...
func (r *storageService) DeleteContainer(idOrName string) error {
	...
	container, err := r.storageImageServer.store.Container(idOrName)
	if err != nil {
		return err
	}
	err = r.storageImageServer.store.DeleteContainer(container.ID)
	if err != nil {
...
func (r *storageService) SetContainerMetadata(idOrName string, metadata RuntimeContainerMetadata) error {
	...
	return r.storageImageServer.store.SetMetadata(idOrName, string(mdata))
}

func (r *storageService) GetContainerMetadata(idOrName string) (RuntimeContainerMetadata, error) {
	metadata := RuntimeContainerMetadata{}
	mdata, err := r.storageImageServer.store.Metadata(idOrName)
	if err != nil {
		return metadata, err
	}
...
func (r *storageService) StartContainer(idOrName string) (string, error) {
	...
	mountPoint, err := r.storageImageServer.store.Mount(container.ID, metadata.MountLabel)
	if err != nil {
...
func (r *storageService) StopContainer(idOrName string) error {
	...
	err = r.storageImageServer.store.Unmount(container.ID)
	if err != nil {
...
func (r *storageService) GetWorkDir(id string) (string, error) {
	container, err := r.storageImageServer.store.Container(id)
	if err != nil {
		if errors.Cause(err) == storage.ErrContainerUnknown {
			return "", ErrInvalidContainerID
		}
	...
	return r.storageImageServer.store.ContainerDirectory(container.ID)
}
...
func (r *storageService) GetRunDir(id string) (string, error) {
	...
	return r.storageImageServer.store.ContainerRunDirectory(container.ID)
}
...
// on those images.
//func GetRuntimeService(storageImageServer ImageServer) RuntimeServer {
